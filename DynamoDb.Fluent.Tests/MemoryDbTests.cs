...
namespace DynamoDb.Fluent.Tests
{
    ...
    public class MemoryDbTests
    {
        private static IMyDynamoContext context;
             ...
             {
                 var (items, count, _) = await context.Users.Query().WithPrimaryKey(userId).WithSecondaryKey().Equal("User")
             ...
             public async Task TestIndex()
             {
                 var result = await context.OwnedEntities.WithIndex("OwnerId-Type-Index").Query()
                     .WithPrimaryKey(userId).WithSecondaryKey().Equal("OwnedEntity") .Get();
